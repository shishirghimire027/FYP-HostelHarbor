...
function UpdateHostelList() {
  ...
  useEffect(() => {
    axios
      .get("http://localhost:3001/getHostel/"+id)
  ...
  const Update = (e) => {
    e.preventDefault();
    axios
      .put("http://localhost:3001/UpdateHostelList/"+id, {
        Hostel_Name,
        Hostel_Type,
        Hostel_Location,
        Manager_Name,
      ...
      .then((result) => {
        console.log(result);
        navigate("/AdminHostelList"); // Providing the absolute path
      })
      .catch((err) => console.log(err));
  };
...
export default  UpdateHostelList;
